...
import com.lib.common.eventbus.handler.AsyncEventHandler;
import com.lib.common.eventbus.handler.DefaultEventHandler;
import com.lib.common.eventbus.handler.EventHandler;
import com.lib.common.eventbus.handler.UIThreadEventHandler;
import com.lib.common.eventbus.matchpolicy.DefaultMatchPolicy;
import com.lib.common.eventbus.matchpolicy.MatchPolicy;
 ...
 * <p/>
 * 注意 : 如果发布的事件的参数类型是订阅的事件参数的子类,订阅函数默认也会被执行。例如您在订阅函数中订阅的是List<String>类型的事件,
 * 但是在发布时发布的是ArrayList<String>的事件,
 * 因此List<String>是一个泛型抽象,而ArrayList<String>才是具体的实现
 * ,因此这种情况下订阅函数也会被执行。如果您需要订阅函数能够接收到的事件类型必须严格匹配 ,您可以构造一个EventBusConfig对象,
