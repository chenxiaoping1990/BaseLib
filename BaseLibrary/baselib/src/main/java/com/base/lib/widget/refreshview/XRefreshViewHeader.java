...
public class XRefreshViewHeader extends LinearLayout implements IHeaderCallBack {
    ...
    public void setRefreshTime(long lastRefreshTime) {
        ...
        if (minutes < 1) {
            ...
            refreshTimeText = RefreshUtils.format(refreshTimeText, minutes);
        } else if (minutes < 60 * 24) {
            refreshTimeText = resources
                    .getString(R.string.xrefreshview_refresh_hours_ago);
            refreshTimeText = RefreshUtils.format(refreshTimeText, minutes / 60);
        } else {
            refreshTimeText = resources
                    .getString(R.string.xrefreshview_refresh_days_ago);
            refreshTimeText = RefreshUtils.format(refreshTimeText, minutes / 60 / 24);
        }
