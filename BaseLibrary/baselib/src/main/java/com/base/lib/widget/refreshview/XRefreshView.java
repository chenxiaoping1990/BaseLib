package com.base.lib.widget.refreshview;
...
import com.base.lib.support.logger.Logger;
import com.base.lib.widget.refreshview.callback.IFooterCallBack;
import com.base.lib.widget.refreshview.callback.IHeaderCallBack;
import com.base.lib.widget.refreshview.listener.OnBottomLoadMoreTime;
import com.base.lib.widget.refreshview.listener.OnTopRefreshTime;
import com.base.lib.widget.refreshview.utils.RefreshUtils;
...
public class XRefreshView extends LinearLayout {
    ...
    private void dealAddHeaderView() {
        if (indexOfChild(mHeaderView) == -1) {
            RefreshUtils.removeViewFromParent(mHeaderView);
    ...
    private void dealAddFooterView() {
        if (indexOfChild(mFooterView) == -1) {
            if (needAddFooterView()) {
                RefreshUtils.removeViewFromParent(mFooterView);
                try {
...
//        if(mHolder.mOffsetY!=0)return;
        Logger.e("onLayout mHolder.mOffsetY=" + mHolder.mOffsetY);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Logger.e("isTop=" + mContentView.isTop() + ";isBottom=" + mContentView.isBottom());
                ...
                if (mHolder.hasHeaderPullDown()) {
                    ...
                    if (!mStopingRefresh) {
                        if (mEnablePullLoad && !isEmptyViewShowing() && needAddFooterView() && !mHasLoadComplete) {
                            ...
                            startScroll(offset, RefreshUtils.computeScrollVerticalDuration(offset, getHeight()));
                        }
                    }
                }
                mLastY = -1; // reset
    ...
    public void setHeadMoveLargestDistence(int headMoveDistence) {
        if (headMoveDistence <= 0) {
            int ScreenHeight = RefreshUtils.getScreenSize(getContext()).y;
    ...
    private void resetHeaderHeight() {
        ...
        if (mPullRefreshing) {
            ...
            startScroll(offsetY, RefreshUtils.computeScrollVerticalDuration(offsetY, getHeight()));
        } else {
            offsetY = 0 - mHolder.mOffsetY;
            startScroll(offsetY, RefreshUtils.computeScrollVerticalDuration(offsetY, getHeight()));
        }
        Logger.e("resetHeaderHeight offsetY=" + offsetY);
    }
...
//            headerMovePercent = headerMovePercent > 1 ? 1 : headerMovePercent;
    ...
    public void stopRefresh(boolean success) {
        Logger.e("stopRefresh mPullRefreshing=" + mPullRefreshing);
    ...
    protected void resetLayout() {
        ...
        if (mHolder.mOffsetY != 0 && !mStopingRefresh) {
            startScroll(-mHolder.mOffsetY, RefreshUtils.computeScrollVerticalDuration(mHolder.mOffsetY, getHeight()));
        }
    }
...
//        mFooterCallBack.onStateRefreshing();
    ...
    private Runnable mRunnable = new Runnable() {
        ...
        public void run() {
            if (mScroller.computeScrollOffset()) {
                ...
                Logger.e("currentY=" + currentY + ";mHolder.mOffsetY=" + mHolder.mOffsetY);
                ...
                if (mHolder.mOffsetY == 0) {
                    ...
                    if (mStopingRefresh && !mPullLoading && !mPullRefreshing) {
                        startScroll(-currentY, RefreshUtils.computeScrollVerticalDuration(currentY, getHeight()));
                    }
                }
            }
        }
    ...
    public void setEmptyView(View emptyView) {
        RefreshUtils.removeViewFromParent(emptyView);
