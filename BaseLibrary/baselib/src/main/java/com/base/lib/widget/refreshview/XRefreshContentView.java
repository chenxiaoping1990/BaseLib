...
import com.base.lib.widget.refreshview.recyclerview.BaseRecyclerAdapter;
import com.base.lib.widget.refreshview.recyclerview.XSpanSizeLookup;
...
public class XRefreshContentView implements OnScrollListener, OnTopRefreshTime, OnBottomLoadMoreTime {
    ...
    public void onRecyclerViewScrolled(RecyclerView recyclerView, BaseRecyclerAdapter adapter, int dx, int dy, boolean force) {
        ...
        Logger.e("test pre onScrolled mIsLoadingMore=" + mIsLoadingMore);
        if (onRecyclerViewTop()) {
            if (RefreshUtils.isRecyclerViewFullscreen(recyclerView)) {
//                        addFooterView(true);
            } else {
                if (mHideFooter) {
            ...
            if (!(manager instanceof GridLayoutManager)) {
                ...
                if (footerView != null) {
                    ...
                    if (layoutParams instanceof StaggeredGridLayoutManager.LayoutParams) {
                        RefreshUtils.setFullSpan((StaggeredGridLayoutManager.LayoutParams) layoutParams);
...
//        mTotalItemCount = 0;
    ...
    private void dealRecyclerViewNotFullScreen() {
        RecyclerView recyclerView = (RecyclerView) child;
        if (onRecyclerViewTop() && !RefreshUtils.isRecyclerViewFullscreen(recyclerView) && child instanceof RecyclerView && mFooterCallBack != null && isFooterEnable()) {
            mFooterCallBack.onStateReady();
            mFooterCallBack.callWhenNotAutoLoadMore(mParent);
            if (!mFooterCallBack.isShowing()) {
                mFooterCallBack.show(true);
